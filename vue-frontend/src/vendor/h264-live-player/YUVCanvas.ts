...
export default class YUVCanvas extends Canvas {
    ...
    constructor(readonly canvas: HTMLCanvasElement, readonly size: Size) {
        ...
        this.canvasBuffer = this.canvasCtx.createImageData(size.width, size.height);
    }
    ...
    public decode(buffer: Uint8Array, width: number, height: number): void {
        ...
        // If resolution changed, update canvas and buffer
        if (width !== this.currentWidth || height !== this.currentHeight) {
            console.log('YUVCanvas: Resolution changed from', this.currentWidth, 'x', this.currentHeight, 'to', width, 'x', height);
